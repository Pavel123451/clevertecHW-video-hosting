...
public class AvatarUpdateHandler implements ChannelUpdateHandler {
    @Override
    public void handle(Channel channel, ChannelUpdateDto updateChannel) {
        if (updateChannel.getAvatar() != null && !updateChannel.getAvatar().isBlank()) {
            channel.setAvatar(updateChannel.getAvatar());
        }
    }
}
